...
void Array_insert(struct Array *array, char* element) {
    int left = 0;
	int right = Array_size(array) -1;
    if (Array_size(array)){
	while (left <= right) {
		int mid = left+(right-left)/2;
		if (strcmp(element, Array_get(array, mid))<0) right = mid-1;
		if (strcmp(element, Array_get(array, mid))>0) left = mid+1;
		if (!strcmp(element, Array_get(array, mid))) return;
	}
    }
    char* dupelement = strdup(element);
    Array_append(array, element); 
    for (int i = Array_size(array)-1 ; i>left ; i--) {
        Array_put(array, i, Array_get(array,i - 1));
    }
    Array_put(array, left, dupelement);
}

ssize_t Array_binSearch(struct Array *array, char* element) {
	int left = 0;
	...
	while (left <= right) {
		...
		if (strcmp(element, Array_get(array, mid))<0) right = mid-1;
		if (strcmp(element, Array_get(array, mid))>0) left = mid+1;
		if (!strcmp(element, Array_get(array, mid))) return mid;
	}
